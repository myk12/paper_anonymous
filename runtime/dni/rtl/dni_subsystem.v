 ...
 * - schedule executor
 * - sibling processor-side stand-in engines
 * - generic host/processor <-> MAC datapath wrapper
 ...
 *   0x0098  RX_WINDOW_STATUS    {rx_exec_valid, rx_enabled, rx_window_active}
 *   0x009C  RX_CURRENT_ENTRY_PTR
 *   0x00A0  RX_ACTIVE_APP_INFO  {plane_id, app_id, opcode}
 *   0x00A4  RX_ACTIVE_CONTEXT
 *   0x00A8  RX_ACTIVE_START_TIME_LO
 *   0x00AC  RX_ACTIVE_START_TIME_HI
 *   0x00B0  RX_ACTIVE_END_TIME_LO
 *   0x00B4  RX_ACTIVE_END_TIME_HI
 *   0x00B8  RX_ACTIVE_ENTRY_META
 *
 * Compatibility / legacy ABI:
 *   0x000C  CTRL_ETHERTYPE      retained for software compatibility only
 *   0x0018  LEGACY_WINDOW_PER   retained for software compatibility only
 *   0x0080  LEGACY_RX_HIT_COUNT
 *   0x0084  LEGACY_RX_PASS_COUNT
 *   0x0088  LEGACY_TX_INJ_COUNT
 *   0x0090  LEGACY_ERR_COUNT
 *   0x0094  LEGACY_CNT_CLEAR
 *
 * App-specific ABI (expected to move behind per-app windows over time):
 ...
 *   0x9000 + entry*32 + word*4  AI trace table (app-specific)
 *
 * Execution-table entry layout consumed by tt_scheduler:
...
// ----------------------------------------------------------------------
// Runtime-core control registers
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Compatibility / legacy control registers
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// App-specific control registers
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Runtime-core executor state
// ----------------------------------------------------------------------
wire [31:0] exec_schedule_rd_data;
wire [31:0] rx_exec_schedule_rd_data;
wire [31:0] exec_tx_word_start_lo;
wire [31:0] exec_tx_word_start_hi;
wire [31:0] exec_tx_word_end_lo;
wire [31:0] exec_tx_word_end_hi;
wire [31:0] exec_tx_word_meta;
wire [31:0] exec_tx_word_route;
wire [31:0] exec_tx_word_flow;
wire [31:0] exec_rx_word_start_lo;
wire [31:0] exec_rx_word_start_hi;
wire [31:0] exec_rx_word_end_lo;
wire [31:0] exec_rx_word_end_hi;
wire [31:0] exec_rx_word_meta;
wire [31:0] exec_rx_word_route;
wire [31:0] exec_rx_word_flow;
...
// ----------------------------------------------------------------------
// App-specific status and table readback
// ----------------------------------------------------------------------
...
localparam integer PROCESSOR_APP_COUNT = 3;

wire [AXIS_IF_DATA_WIDTH-1:0]           processor_tx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]           processor_tx_tkeep;
wire                                    processor_tx_tvalid;
wire                                    processor_tx_tlast;
wire [AXIS_IF_TX_USER_WIDTH-1:0]        processor_tx_tuser;
wire                                    processor_tx_tready;
wire                                    processor_tx_valid;
wire [AXIS_IF_DATA_WIDTH-1:0]           processor_rx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]           processor_rx_tkeep;
wire                                    processor_rx_tvalid;
wire                                    processor_rx_tlast;
wire [AXIS_IF_RX_USER_WIDTH-1:0]        processor_rx_tuser;
wire                                    processor_rx_tready;
wire [AXIS_IF_TX_DEST_WIDTH-1:0]        processor_tx_tdest;
wire                                    subsystem_enable = reg_ctrl_enable && !reg_ctrl_reset;

wire [PROCESSOR_APP_COUNT*64-1:0]                 proc_app_tx_window_id;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_tx_window_open_pulse;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_tx_commit_start_pulse;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_tx_window_close_pulse;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_tx_allowed;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_tx_active;
wire [PROCESSOR_APP_COUNT*8-1:0]                  proc_app_tx_opcode;
wire [PROCESSOR_APP_COUNT*16-1:0]                 proc_app_tx_context_id;
wire [PROCESSOR_APP_COUNT*64-1:0]                 proc_app_rx_window_id;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_rx_window_open_pulse;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_rx_commit_start_pulse;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_rx_window_close_pulse;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_rx_enabled;
wire [PROCESSOR_APP_COUNT-1:0]                    proc_app_rx_active;
wire [PROCESSOR_APP_COUNT*8-1:0]                  proc_app_rx_opcode;
wire [PROCESSOR_APP_COUNT*16-1:0]                 proc_app_rx_context_id;

wire [AXIS_IF_DATA_WIDTH-1:0]                     cons_tx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]                     cons_tx_tkeep;
wire                                              cons_tx_tvalid;
wire                                              cons_tx_tlast;
wire [AXIS_IF_TX_USER_WIDTH-1:0]                  cons_tx_tuser;
wire                                              cons_tx_tready;
wire [AXIS_IF_DATA_WIDTH-1:0]                     cons_rx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]                     cons_rx_tkeep;
wire                                              cons_rx_tvalid;
wire                                              cons_rx_tlast;
wire [AXIS_IF_RX_USER_WIDTH-1:0]                  cons_rx_tuser;
wire                                              cons_rx_tready;

wire [AXIS_IF_DATA_WIDTH-1:0]                     ai_tx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]                     ai_tx_tkeep;
wire                                              ai_tx_tvalid;
wire                                              ai_tx_tlast;
wire [AXIS_IF_TX_USER_WIDTH-1:0]                  ai_tx_tuser;
wire                                              ai_tx_tready;
wire [AXIS_IF_DATA_WIDTH-1:0]                     ai_rx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]                     ai_rx_tkeep;
wire                                              ai_rx_tvalid;
wire                                              ai_rx_tlast;
wire [AXIS_IF_RX_USER_WIDTH-1:0]                  ai_rx_tuser;
wire                                              ai_rx_tready;

wire [AXIS_IF_DATA_WIDTH-1:0]                     adapter_tx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]                     adapter_tx_tkeep;
wire                                              adapter_tx_tvalid;
wire                                              adapter_tx_tlast;
wire [AXIS_IF_TX_USER_WIDTH-1:0]                  adapter_tx_tuser;
wire                                              adapter_tx_tready;
wire [AXIS_IF_DATA_WIDTH-1:0]                     adapter_rx_tdata;
wire [AXIS_IF_KEEP_WIDTH-1:0]                     adapter_rx_tkeep;
wire                                              adapter_rx_tvalid;
wire                                              adapter_rx_tlast;
wire [AXIS_IF_RX_USER_WIDTH-1:0]                  adapter_rx_tuser;
wire                                              adapter_rx_tready;
wire                                              adapter_done;
wire                                              adapter_error;
wire                                              adapter_halt;
wire                                              adapter_busy;
wire [31:0]                                       adapter_status;

wire [PROCESSOR_APP_COUNT*AXIS_IF_DATA_WIDTH-1:0]    app_slot_tx_tdata;
wire [PROCESSOR_APP_COUNT*AXIS_IF_KEEP_WIDTH-1:0]    app_slot_tx_tkeep;
wire [PROCESSOR_APP_COUNT-1:0]                       app_slot_tx_tvalid;
wire [PROCESSOR_APP_COUNT-1:0]                       app_slot_tx_tlast;
wire [PROCESSOR_APP_COUNT*AXIS_IF_TX_USER_WIDTH-1:0] app_slot_tx_tuser;
wire [PROCESSOR_APP_COUNT-1:0]                       app_slot_tx_tready;
wire [PROCESSOR_APP_COUNT*AXIS_IF_DATA_WIDTH-1:0]    app_slot_rx_tdata;
wire [PROCESSOR_APP_COUNT*AXIS_IF_KEEP_WIDTH-1:0]    app_slot_rx_tkeep;
wire [PROCESSOR_APP_COUNT-1:0]                       app_slot_rx_tvalid;
wire [PROCESSOR_APP_COUNT-1:0]                       app_slot_rx_tlast;
wire [PROCESSOR_APP_COUNT*AXIS_IF_RX_USER_WIDTH-1:0] app_slot_rx_tuser;
wire [PROCESSOR_APP_COUNT-1:0]                       app_slot_rx_tready;

assign app_slot_tx_tdata[0*AXIS_IF_DATA_WIDTH +: AXIS_IF_DATA_WIDTH] = cons_tx_tdata;
assign app_slot_tx_tdata[1*AXIS_IF_DATA_WIDTH +: AXIS_IF_DATA_WIDTH] = ai_tx_tdata;
assign app_slot_tx_tdata[2*AXIS_IF_DATA_WIDTH +: AXIS_IF_DATA_WIDTH] = adapter_tx_tdata;
assign app_slot_tx_tkeep[0*AXIS_IF_KEEP_WIDTH +: AXIS_IF_KEEP_WIDTH] = cons_tx_tkeep;
assign app_slot_tx_tkeep[1*AXIS_IF_KEEP_WIDTH +: AXIS_IF_KEEP_WIDTH] = ai_tx_tkeep;
assign app_slot_tx_tkeep[2*AXIS_IF_KEEP_WIDTH +: AXIS_IF_KEEP_WIDTH] = adapter_tx_tkeep;
assign app_slot_tx_tvalid[0] = cons_tx_tvalid;
assign app_slot_tx_tvalid[1] = ai_tx_tvalid;
assign app_slot_tx_tvalid[2] = adapter_tx_tvalid;
assign app_slot_tx_tlast[0] = cons_tx_tlast;
assign app_slot_tx_tlast[1] = ai_tx_tlast;
assign app_slot_tx_tlast[2] = adapter_tx_tlast;
assign app_slot_tx_tuser[0*AXIS_IF_TX_USER_WIDTH +: AXIS_IF_TX_USER_WIDTH] = cons_tx_tuser;
assign app_slot_tx_tuser[1*AXIS_IF_TX_USER_WIDTH +: AXIS_IF_TX_USER_WIDTH] = ai_tx_tuser;
assign app_slot_tx_tuser[2*AXIS_IF_TX_USER_WIDTH +: AXIS_IF_TX_USER_WIDTH] = adapter_tx_tuser;
assign cons_tx_tready = app_slot_tx_tready[0];
assign ai_tx_tready = app_slot_tx_tready[1];
assign adapter_tx_tready = app_slot_tx_tready[2];

assign cons_rx_tdata = app_slot_rx_tdata[0*AXIS_IF_DATA_WIDTH +: AXIS_IF_DATA_WIDTH];
assign ai_rx_tdata = app_slot_rx_tdata[1*AXIS_IF_DATA_WIDTH +: AXIS_IF_DATA_WIDTH];
assign adapter_rx_tdata = app_slot_rx_tdata[2*AXIS_IF_DATA_WIDTH +: AXIS_IF_DATA_WIDTH];
assign cons_rx_tkeep = app_slot_rx_tkeep[0*AXIS_IF_KEEP_WIDTH +: AXIS_IF_KEEP_WIDTH];
assign ai_rx_tkeep = app_slot_rx_tkeep[1*AXIS_IF_KEEP_WIDTH +: AXIS_IF_KEEP_WIDTH];
assign adapter_rx_tkeep = app_slot_rx_tkeep[2*AXIS_IF_KEEP_WIDTH +: AXIS_IF_KEEP_WIDTH];
assign cons_rx_tvalid = app_slot_rx_tvalid[0];
assign ai_rx_tvalid = app_slot_rx_tvalid[1];
assign adapter_rx_tvalid = app_slot_rx_tvalid[2];
assign cons_rx_tlast = app_slot_rx_tlast[0];
assign ai_rx_tlast = app_slot_rx_tlast[1];
assign adapter_rx_tlast = app_slot_rx_tlast[2];
assign cons_rx_tuser = app_slot_rx_tuser[0*AXIS_IF_RX_USER_WIDTH +: AXIS_IF_RX_USER_WIDTH];
assign ai_rx_tuser = app_slot_rx_tuser[1*AXIS_IF_RX_USER_WIDTH +: AXIS_IF_RX_USER_WIDTH];
assign adapter_rx_tuser = app_slot_rx_tuser[2*AXIS_IF_RX_USER_WIDTH +: AXIS_IF_RX_USER_WIDTH];
assign app_slot_rx_tready[0] = cons_rx_tready;
assign app_slot_rx_tready[1] = ai_rx_tready;
assign app_slot_rx_tready[2] = adapter_rx_tready;
...
exec_table_inst (
    .clk(clk),
    .rst(rst),
    .i_active_bank(exec_active_bank),
    ...
    .i_tx_eval_entry(exec_current_entry_ptr),
    .i_rx_eval_entry(rx_exec_current_entry_ptr),
    .o_tx_word_start_lo(exec_tx_word_start_lo),
    .o_tx_word_start_hi(exec_tx_word_start_hi),
    .o_tx_word_end_lo(exec_tx_word_end_lo),
    .o_tx_word_end_hi(exec_tx_word_end_hi),
    .o_tx_word_meta(exec_tx_word_meta),
    .o_tx_word_route(exec_tx_word_route),
    .o_tx_word_flow(exec_tx_word_flow),
    .o_rx_word_start_lo(exec_rx_word_start_lo),
    .o_rx_word_start_hi(exec_rx_word_start_hi),
    .o_rx_word_end_lo(exec_rx_word_end_lo),
    .o_rx_word_end_hi(exec_rx_word_end_hi),
    .o_rx_word_meta(exec_rx_word_meta),
    .o_rx_word_route(exec_rx_word_route),
    .o_rx_word_flow(exec_rx_word_flow)
);
...
tt_scheduler_inst (
    ...
    .i_tx_word_start_lo(exec_tx_word_start_lo),
    .i_tx_word_start_hi(exec_tx_word_start_hi),
    .i_tx_word_end_lo(exec_tx_word_end_lo),
    .i_tx_word_end_hi(exec_tx_word_end_hi),
    .i_tx_word_meta(exec_tx_word_meta),
    .i_tx_word_route(exec_tx_word_route),
    .i_tx_word_flow(exec_tx_word_flow),
    .i_rx_word_start_lo(exec_rx_word_start_lo),
    .i_rx_word_start_hi(exec_rx_word_start_hi),
    .i_rx_word_end_lo(exec_rx_word_end_lo),
    .i_rx_word_end_hi(exec_rx_word_end_hi),
    .i_rx_word_meta(exec_rx_word_meta),
    .i_rx_word_route(exec_rx_word_route),
    .i_rx_word_flow(exec_rx_word_flow),
    .o_tx_current_window_id(exec_current_window_id),
    .o_tx_current_entry_ptr(exec_current_entry_ptr),
    .o_tx_window_open_pulse(exec_window_open_pulse),
    .o_tx_window_close_pulse(exec_window_close_pulse),
    .o_tx_commit_start_pulse(exec_commit_start_pulse),
    .o_tx_exec_valid(exec_valid),
    .o_tx_allowed(exec_tx_allowed),
    .o_tx_window_active(exec_window_active),
    .o_tx_target_port(exec_target_port),
    .o_tx_target_queue(exec_target_queue),
    .o_tx_app_id(exec_app_id),
    .o_tx_plane_id(exec_plane_id),
    .o_tx_opcode(exec_opcode),
    .o_tx_context_id(exec_context_id),
    .o_tx_dst_node_id(exec_dst_node_id),
    .o_tx_flow_id(exec_flow_id),
...
processor_runtime #(
    ...
    .APP_COUNT(PROCESSOR_APP_COUNT)
)
processor_runtime_inst (
    .clk(clk),
    .rst(rst),
    .i_enable(subsystem_enable),

    // Schedule execution status and control signals
    ...
    // AXI-Stream interface to CommDatapath
    .s_axis_processor_rx_tdata(processor_rx_tdata),
    .s_axis_processor_rx_tkeep(processor_rx_tkeep),
    .s_axis_processor_rx_tvalid(processor_rx_tvalid),
    .s_axis_processor_rx_tlast(processor_rx_tlast),
    .s_axis_processor_rx_tuser(processor_rx_tuser),
    .s_axis_processor_rx_tready(processor_rx_tready),
    .m_axis_processor_tx_tdata(processor_tx_tdata),
    .m_axis_processor_tx_tkeep(processor_tx_tkeep),
    .m_axis_processor_tx_tvalid(processor_tx_tvalid),
    .m_axis_processor_tx_tready(processor_tx_tready),
    .m_axis_processor_tx_tlast(processor_tx_tlast),
    .m_axis_processor_tx_tuser(processor_tx_tuser),

    // Control and status signals to/from processor apps
    .o_processor_tx_valid(processor_tx_valid),
    .o_app_tx_window_id(proc_app_tx_window_id),
    .o_app_tx_window_open_pulse(proc_app_tx_window_open_pulse),
    .o_app_tx_commit_start_pulse(proc_app_tx_commit_start_pulse),
    .o_app_tx_window_close_pulse(proc_app_tx_window_close_pulse),
    .o_app_tx_allowed(proc_app_tx_allowed),
    .o_app_tx_active(proc_app_tx_active),
    .o_app_tx_opcode(proc_app_tx_opcode),
    .o_app_tx_context_id(proc_app_tx_context_id),
    .o_app_rx_window_id(proc_app_rx_window_id),
    .o_app_rx_window_open_pulse(proc_app_rx_window_open_pulse),
    .o_app_rx_commit_start_pulse(proc_app_rx_commit_start_pulse),
    .o_app_rx_window_close_pulse(proc_app_rx_window_close_pulse),
    .o_app_rx_enabled(proc_app_rx_enabled),
    .o_app_rx_active(proc_app_rx_active),
    .o_app_rx_opcode(proc_app_rx_opcode),
    .o_app_rx_context_id(proc_app_rx_context_id),
    .s_axis_app_tx_tdata(app_slot_tx_tdata),
    .s_axis_app_tx_tkeep(app_slot_tx_tkeep),
    .s_axis_app_tx_tvalid(app_slot_tx_tvalid),
    .s_axis_app_tx_tlast(app_slot_tx_tlast),
    .s_axis_app_tx_tuser(app_slot_tx_tuser),
    .s_axis_app_tx_tready(app_slot_tx_tready),
    .m_axis_app_rx_tdata(app_slot_rx_tdata),
    .m_axis_app_rx_tkeep(app_slot_rx_tkeep),
    .m_axis_app_rx_tvalid(app_slot_rx_tvalid),
    .m_axis_app_rx_tlast(app_slot_rx_tlast),
    .m_axis_app_rx_tuser(app_slot_rx_tuser),
    .m_axis_app_rx_tready(app_slot_rx_tready)
);
...
consensus_node #(
    ...
    .P_LOG_ITEM_LEN(40),
    .P_AXIS_DATA_WIDTH(AXIS_IF_DATA_WIDTH),
    .P_AXIS_KEEP_WIDTH(AXIS_IF_KEEP_WIDTH),
    .P_AXIS_TX_USER_WIDTH(AXIS_IF_TX_USER_WIDTH),
    .P_AXIS_RX_USER_WIDTH(AXIS_IF_RX_USER_WIDTH),
    .P_NODE_ID_WIDTH(8),
    .P_KV_WIDTH(8),
    .P_HDR_ETHERTYPE_OFFSET(12),
    .P_HDR_WINDOW_ID_OFFSET(14),
    .P_HDR_NODE_ID_OFFSET(22),
    .P_HDR_KV_OFFSET(23),
    .P_HDR_PAYLOAD_OFFSET(24),
    .P_DEST_MAC_0(48'h00_0a_35_06_50_94),
    .P_DEST_MAC_1(48'h00_0a_35_06_09_24),
    .P_DEST_MAC_2(48'h00_0a_35_06_0b_84),
    .P_DEST_MAC_3(48'h00_0a_35_06_09_3c),
    .P_DEST_MAC_4(48'h00_0a_35_06_0b_72),
    .P_BROADCAST_MAC(48'hFF_FF_FF_FF_FF_FF)
)
consensus_node_inst (
    .clk(clk),
    .rst_n(!rst),
    .i_enable(subsystem_enable && reg_consensus_enable),
    .i_clear_halt(reg_consensus_clear_halt),
    .i_current_window_id(proc_app_tx_window_id[0*64 +: 64]),
    .i_window_open_pulse(proc_app_tx_window_open_pulse[0]),
    .i_commit_start_pulse(proc_app_tx_commit_start_pulse[0]),
    .i_window_close_pulse(proc_app_tx_window_close_pulse[0]),
    .i_tx_allowed(proc_app_tx_allowed[0]),
    .i_rx_enabled(proc_app_rx_enabled[0]),
    .m_axis_mac_tx_tdata(cons_tx_tdata),
    .m_axis_mac_tx_tkeep(cons_tx_tkeep),
    .m_axis_mac_tx_tvalid(cons_tx_tvalid),
    .m_axis_mac_tx_tlast(cons_tx_tlast),
    .m_axis_mac_tx_tuser(cons_tx_tuser),
    .m_axis_mac_tx_tready(cons_tx_tready),
    .s_axis_mac_rx_tdata(cons_rx_tdata),
    .s_axis_mac_rx_tkeep(cons_rx_tkeep),
    .s_axis_mac_rx_tvalid(cons_rx_tvalid),
    .s_axis_mac_rx_tlast(cons_rx_tlast),
    .s_axis_mac_rx_tuser(cons_rx_tuser),
    .s_axis_mac_rx_tready(cons_rx_tready),
    .s_axis_host_req_data({AXIS_IF_DATA_WIDTH{1'b0}}),
    .s_axis_host_req_keep({AXIS_IF_KEEP_WIDTH{1'b0}}),
    .s_axis_host_req_valid(1'b0),
    .s_axis_host_req_last(1'b0),
    .s_axis_host_req_ready(),
    .m_axis_host_commit_data(),
    .m_axis_host_commit_keep(),
    .m_axis_host_commit_valid(),
    .m_axis_host_commit_last(),
    .m_axis_host_commit_ready(1'b0),
    .o_system_halt(consensus_system_halt),
    .o_debug_state(consensus_debug_state)
);
...
ai_trace_replay #(
    ...
    .P_SRC_MAC(48'h00_0a_35_06_50_94)
)
ai_trace_replay_inst (
    .clk(clk),
    .rst(rst),
    .i_enable(subsystem_enable),
    .i_tx_current_window_id(proc_app_tx_window_id[1*64 +: 64]),
    .i_tx_window_open_pulse(proc_app_tx_window_open_pulse[1]),
    .i_tx_window_active(proc_app_tx_active[1] && proc_app_tx_opcode[1*8 +: 8] == `DNI_OP_AI_TX),
    .i_tx_allowed(proc_app_tx_allowed[1]),
    .i_tx_context_id(proc_app_tx_context_id[1*16 +: 16]),
    .i_rx_current_window_id(proc_app_rx_window_id[1*64 +: 64]),
    .i_rx_window_open_pulse(proc_app_rx_window_open_pulse[1]),
    .i_rx_window_active(proc_app_rx_active[1] && proc_app_rx_opcode[1*8 +: 8] == `DNI_OP_AI_RX),
    .i_rx_enabled(proc_app_rx_enabled[1]),
    .i_rx_context_id(proc_app_rx_context_id[1*16 +: 16]),
    .cfg_enable(reg_ai_enable),
    .cfg_wr_en(ai_trace_wr_sel),
    .cfg_wr_entry(ai_trace_wr_entry),
    .cfg_wr_word(ai_trace_wr_word),
    .cfg_wr_data(reg_wr_data),
    .cfg_rd_entry(ai_trace_rd_entry),
    .cfg_rd_word(ai_trace_rd_word),
    .cfg_rd_data(ai_trace_rd_data),
    .o_pkt_sent_count(ai_pkt_sent_count),
    .o_rx_pkt_count(ai_rx_pkt_count),
    .o_rx_byte_count(ai_rx_byte_count),
    .o_rx_match_count(ai_rx_match_count),
    .o_rx_drop_count(ai_rx_drop_count),
    .m_axis_tx_tdata(ai_tx_tdata),
    .m_axis_tx_tkeep(ai_tx_tkeep),
    .m_axis_tx_tvalid(ai_tx_tvalid),
    .m_axis_tx_tready(ai_tx_tready),
    .m_axis_tx_tlast(ai_tx_tlast),
    .m_axis_tx_tuser(ai_tx_tuser),
    .s_axis_rx_tdata(ai_rx_tdata),
    .s_axis_rx_tkeep(ai_rx_tkeep),
    .s_axis_rx_tvalid(ai_rx_tvalid),
    .s_axis_rx_tlast(ai_rx_tlast),
    .s_axis_rx_tuser(ai_rx_tuser),
    .s_axis_rx_tready(ai_rx_tready)
);

processor_adapter_stub #(
    .AXIS_DATA_WIDTH(AXIS_IF_DATA_WIDTH),
    .AXIS_KEEP_WIDTH(AXIS_IF_KEEP_WIDTH),
    .AXIS_TX_USER_WIDTH(AXIS_IF_TX_USER_WIDTH),
    .AXIS_RX_USER_WIDTH(AXIS_IF_RX_USER_WIDTH),
...
processor_adapter_stub_inst (
    .clk(clk),
    .rst(rst),
    .i_enable(subsystem_enable),
    .i_tx_window_id(proc_app_tx_window_id[2*64 +: 64]),
    .i_tx_window_open_pulse(proc_app_tx_window_open_pulse[2]),
    .i_tx_window_close_pulse(proc_app_tx_window_close_pulse[2]),
    .i_tx_commit_start_pulse(proc_app_tx_commit_start_pulse[2]),
    .i_tx_allowed(proc_app_tx_allowed[2]),
    .i_tx_active(proc_app_tx_active[2]),
    .i_tx_opcode(proc_app_tx_opcode[2*8 +: 8]),
    .i_tx_context_id(proc_app_tx_context_id[2*16 +: 16]),
    .i_rx_window_id(proc_app_rx_window_id[2*64 +: 64]),
    .i_rx_window_open_pulse(proc_app_rx_window_open_pulse[2]),
    .i_rx_window_close_pulse(proc_app_rx_window_close_pulse[2]),
    .i_rx_commit_start_pulse(proc_app_rx_commit_start_pulse[2]),
    .i_rx_enabled(proc_app_rx_enabled[2]),
    .i_rx_active(proc_app_rx_active[2]),
    .i_rx_opcode(proc_app_rx_opcode[2*8 +: 8]),
    .i_rx_context_id(proc_app_rx_context_id[2*16 +: 16]),
    .m_axis_tx_tdata(adapter_tx_tdata),
    .m_axis_tx_tkeep(adapter_tx_tkeep),
    .m_axis_tx_tvalid(adapter_tx_tvalid),
    .m_axis_tx_tready(adapter_tx_tready),
    .m_axis_tx_tlast(adapter_tx_tlast),
    .m_axis_tx_tuser(adapter_tx_tuser),
    .s_axis_rx_tdata(adapter_rx_tdata),
    .s_axis_rx_tkeep(adapter_rx_tkeep),
    .s_axis_rx_tvalid(adapter_rx_tvalid),
    .s_axis_rx_tready(adapter_rx_tready),
    .s_axis_rx_tlast(adapter_rx_tlast),
    .s_axis_rx_tuser(adapter_rx_tuser),
    .o_done(adapter_done),
    .o_error(adapter_error),
    .o_halt(adapter_halt),
    .o_busy(adapter_busy),
    .o_status(adapter_status)
);
...
comm_datapath #(
    ...
    .AXIS_TX_USER_WIDTH(AXIS_IF_TX_USER_WIDTH),
    .AXIS_RX_USER_WIDTH(AXIS_IF_RX_USER_WIDTH),
    .P_PROCESSOR_ETHERTYPE_0(16'h88B5),
    .P_PROCESSOR_ETHERTYPE_1(16'h88B6),
...
comm_datapath_inst (
    ...
    .s_axis_processor_tx_tdata(processor_tx_tdata),
    .s_axis_processor_tx_tkeep(processor_tx_tkeep),
    .s_axis_processor_tx_tvalid(processor_tx_tvalid),
    .s_axis_processor_tx_tlast(processor_tx_tlast),
    .s_axis_processor_tx_tuser(processor_tx_tuser),
    .s_axis_processor_tx_tready(processor_tx_tready),
    .i_processor_tx_valid(processor_tx_valid),
    .o_processor_tx_selected(),
    ...
    .m_axis_processor_rx_tdata(processor_rx_tdata),
    .m_axis_processor_rx_tkeep(processor_rx_tkeep),
    .m_axis_processor_rx_tvalid(processor_rx_tvalid),
    .m_axis_processor_rx_tlast(processor_rx_tlast),
    .m_axis_processor_rx_tuser(processor_rx_tuser),
    .m_axis_processor_rx_tready(processor_rx_tready)
);
...
dni_tx_dest_format_inst (
    .i_plane_id(exec_plane_id),
    .i_target_port(exec_target_port),
    .o_tdest(processor_tx_tdest)
);

assign m_axis_if_tx_tid = s_axis_if_tx_tid;
assign m_axis_if_tx_tdest = processor_tx_valid ? processor_tx_tdest : s_axis_if_tx_tdest;
